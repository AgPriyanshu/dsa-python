...
class Solution:
  def recur(self,wt,val,n,W,i,w,dp):
    ...
    if sum_w <= W:
      ...
      pick_same = val[i] + self.recur(wt,val,n,W,i,sum_w,dp)
    
    not_pick = 0 + self.recur(wt,val,n,W,i+1,w,dp)

    dp[f'{i}-{w}'] = max(pick,pick_same,not_pick)

    return max(pick,pick_same,not_pick)
...
if __name__ == "__main__":
  ...
  W = 10
  n = len(wt)
  print(Solution().unboundedKnapsack(wt,val,n,W))